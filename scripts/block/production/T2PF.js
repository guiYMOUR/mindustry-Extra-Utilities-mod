...
lib.setBuildingSimple(T2PF, AttributeSmelter.AttributeSmelterBuild, {
    draw(){
        var x = this.x;
        var y = this.y;
        Draw.rect(Core.atlas.find("btm-T2-PF-bottom"),x,y);
        Draw.rect(Core.atlas.find("btm-T2-PF-s"), x, y, 0 + this.totalProgress * 3)
        Draw.color(Color.valueOf("fff3d6"));
        Draw.alpha(this.progress);
        Draw.rect(Core.atlas.find("btm-T2-PF-item"), x, y);
        Draw.color();
        Draw.color(cor);
        ...
        Draw.reset();
        Draw.rect(Core.atlas.find("btm-T2-PF-a"),x,y);
    },
});
